...
BEGIN
    ...
    -- CRM CUSTOMER INFO
		...
		SELECT 
			...
			CAST(DATEADD(DAY,-1,LEAD(prd_start_dt)
		...
		SELECT
			...
			CASE WHEN sls_order_dt = 0 OR LEN(sls_order_dt) ! = 8 THEN NULL
				ELSE CAST(CAST(sls_order_dt AS VARCHAR)AS DATE)   --INT TO DATE CONVERSION
			END AS sls_order_dt,

			CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) ! = 8 THEN NULL
				ELSE CAST(CAST(sls_ship_dt AS VARCHAR)AS DATE)   --INT TO DATE CONVERSION
			END AS sls_ship_dt,

			CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) ! = 8 THEN NULL
				ELSE CAST(CAST(sls_due_dt AS VARCHAR)AS DATE)   --INT TO DATE CONVERSION
			END AS sls_due_dt,
